...
from fastapi import APIRouter, UploadFile, File, Depends, HTTPException, status, Request, WebSocket
from sqlmodel import Session
from typing import List

from app.api.database import get_session
from app.api.services import process_candidate, get_all_candidates, handle_webrtc_session
...
async def webrtc_endpoint(websocket: WebSocket):
    """
    Эндпоинт для сессии голосовой связи с ИИ интервьюером.
    Сессия поддерживается до тех пор, пока одной из сторон не прервётся WebRTC соединение.
    ...
    """
    logger = getLogger(settings.LOGGER)
    await handle_webrtc_session(websocket)
