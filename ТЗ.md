...
3.  **Илья  Степанов** (ML Engineer & Frontend)
...
Гарантирует, что LLM (Андрей), ML (Илья ) и UI (Илья ) используют одни константы.
```python
...
### Илья  Степанов: ML Engineer & Frontend
...
genai-project/
├── data/
│   ├── train_dataset.csv       # Илья  (генерация)
...
*   **ML/UI (Илья ):** Scikit-learn / CatBoost, Streamlit.
